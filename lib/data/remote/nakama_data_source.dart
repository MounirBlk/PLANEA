...
import 'package:planea/domain/entities/server_config_entity.dart';
...
class NakamaDataSource {
  ...
  static final _grpcPort = int.parse(
    const String.fromEnvironment('NAKAMA_GRPC_PORT', defaultValue: '7349'),
  );
  static final _httpPort = int.parse(
  ...
  final client = getNakamaClient(
    ...
    grpcPort: _grpcPort, // optional
    httpPort: _httpPort, // optional
  );
  ...
  Stream<MatchEvent> onMatchEvent(String matchId) async* {
    await for (final data in _websocketClient.onMatchData) {
      ...
      final index = MatchEventOpCode.values.indexWhere(
        (element) => element.opCode == data.opCode,
      );
      if (index == -1) {
  ...
  Future<MatchResultEntity> getMatchResult(String matchId) async {
    final response = await client.rpc(
      session: _currentSession,
      id: 'get_match_result',
      payload: _makePayload({'matchId': matchId}),
    );
