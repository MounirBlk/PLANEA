            ...
            children: [
              ...
              ...showingItems.map(
                (item) => item is _ScoreRowPlayerItem
                    ...
                    : _MultiplayerDotsRowWidget(screenSize: screenSize),
              ),
            ],
          ),
        ),
...
class _MultiplayerDotsRowWidget extends StatelessWidget {
  const _MultiplayerDotsRowWidget({required this.screenSize});
...
class _MultiplayerScoreRowWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final planeaColor = AppColors.getPlaneaColor(score.planeaType);
    ...
    final planeaSize = switch (screenSize) {
      ScreenSize.extraSmall || ScreenSize.small => 16.0,
        ...
        child: Row(
          children: [
            SizedBox(
              ...
              child: Text(
                score.rank.toString(),
                style: TextStyle(color: Colors.white, fontSize: rankTextSize),
                textAlign: TextAlign.right,
              ),
            ),
            const SizedBox(width: 12),
            Image.asset(
              score.planeaType.pngAssetName,
              width: planeaSize,
              height: planeaSize,
            ),
            ...
            Expanded(
              child: Text(
                showingName,
                style: TextStyle(color: planeaColor, fontSize: normalTextSize),
              ),
            ),
            SizedBox(
...
class PlaneaedBorderPainter extends CustomPainter {
  final Color color;
  final double strokeWidth;
  final double planeaWidth;
  final double planeaGap;
  final bool drawTop;
  ...
  PlaneaedBorderPainter({
    required this.color,
    this.strokeWidth = 2.0,
    this.planeaWidth = 5.0,
    this.planeaGap = 3.0,
    this.drawTop = true,
    this.drawRight = true,
  ...
  void paint(Canvas canvas, Size size) {
    if (_path == null) {
      ...
      // Create the planeaed path
      _path = _createPlaneaedPath(path, planeaWidth, planeaGap);
    }
    canvas.drawPath(_path!, _paint);
  }

  Path _createPlaneaedPath(Path path, double planeaWidth, double planeaGap) {
    final planeaedPath = Path();
    double distance = 0.0;

    for (var metric in path.computeMetrics()) {
      while (distance < metric.length) {
        final segmentLength = planeaWidth + planeaGap;
        final extractLength = distance + planeaWidth;
        planeaedPath.addPath(
...
class PlaneaContainer extends StatelessWidget {
  final Widget child;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: CustomPaint(
        painter: PlaneaedBorderPainter(
          color: borderColor,
          strokeWidth: strokeWidth,
          planeaWidth: planeaWidth,
          planeaGap: planeaGap,
          drawTop: drawTop,
          drawRight: drawRight,
