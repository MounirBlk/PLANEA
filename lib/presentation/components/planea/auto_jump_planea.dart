...
class AutoJumpPlanea extends Component with ParentIsA<Planea> {
  PipePair? _nextPipe;
  ...
  PlaneaGame get gameRef => parent.game;
  ...
  AutoJumpPlanea({this.jumpEvery = 0.2}) : canJumpIn = jumpEvery;
  ...
  PipePair? _getNextPipe() {
    ...
    final pipes = gameRef.world.rootComponent.children
        .whereType<PipePair>()
        .toList();
    if (pipes.isEmpty) {
      return null;
    ...
    if (debugMode) {
      ...
      if (_nextPipe != null) {
        ...
        canvas.drawLine(
          ...
          Offset(targetX - position.x, targetY - position.y),
          Paint()
